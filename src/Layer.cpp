...
Layer::Layer(SDL_Renderer* renderer, Type type, unsigned int level, int parallaxVel) : _renderer(renderer), _type(type), _level(level), _parallaxVel(parallaxVel) {
    std::string fileName;
    ...
    _texture = TextureManager::loadTexture(_renderer, ss.str());
    if(!_texture) {
        std::cerr << "Error: failed to initialize layer texture." << std::endl;
    }
}

Layer::Layer(SDL_Renderer* renderer, Type type, unsigned int level, unsigned int tileSize, i2v& windowSize) : _renderer(renderer), _type(type), _level(level), _tileSize(tileSize), _windowSize(windowSize), _texture(nullptr) {
    std::string fileName;
...
void Layer::renderNonGrid(Camera* camera) {
    if(!_texture) {
        return;
    }
    ...
    // TODO: to remove
    if(_type == BACKGROUND) _texture = TextureManager::loadTexture(_renderer, "../assets/levels/level_001/textures/background.png");
    if(_type == FAR) _texture = TextureManager::loadTexture(_renderer, "../assets/levels/level_001/textures/far.png");
...
void Layer::renderGrid(Camera* camera) {
    int x = 0; 
    int y = 0;
